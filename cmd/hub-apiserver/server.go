...
func invoke(ctx *cli.Context) error {
	...
	config := server.Config{
		...
		Hub: hub.Config{
			Authenticators:          ctx.StringSlice("hub-authentication-plugin"),
			...
			HMAC:                    ctx.String("hub-hmac"),
			PublicAPIURL:            ctx.String("api-public-url"),
			PublicHubURL:            ctx.String("hub-public-url"),
	...
	log.Info("attempting to start the hub-apiserver")

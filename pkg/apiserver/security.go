...
func (s *securityHandler) Register(i kore.Interface, builder utils.PathBuilder) (*restful.WebService, error) {
	...
	ws.Route(
		withAllNonValidationErrors(ws.GET("")).To(s.listSecurityScans).
			...
			Returns(http.StatusOK, "A collection of security scans", securityv1.ScanResultList{}),
	)
	...
	ws.Route(
		withAllNonValidationErrors(ws.GET("{group}/{version}/{kind}/{namespace}/{name}")).To(s.getSecurityScanForResource).
			...
			Returns(http.StatusOK, "Latest security scan for this resource", securityv1.ScanResult{}),
	)

	ws.Route(
		withAllNonValidationErrors(ws.GET("{group}/{version}/{kind}/{namespace}/{name}/history")).
			...
			Param(ws.PathParameter("namespace", "Is the namespace of the resource")).
			Param(ws.PathParameter("name", "Is the name of the resource")).
			Returns(http.StatusOK, "Latest security scan for this resource", securityv1.ScanResultList{}),
	)
	...
	ws.Route(
		withAllNonValidationErrors(ws.GET("{id}")).To(s.getSecurityScan).
			...
			Returns(http.StatusOK, "Security scan", securityv1.ScanResult{}),
	)

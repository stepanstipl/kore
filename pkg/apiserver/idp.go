...
func (id *idpHandler) Register(i kore.Interface, builder utils.PathBuilder) (*restful.WebService, error) {
	...
	ws.Route(
		ws.GET("/types").To(id.getTypes).
			...
			Filter(filters.NewAuditingFilter(i.Audit, path.Base(), "ListIDPTypes")).
	...
	ws.Route(
		ws.GET("/default").To(id.getDefaultIDP).
			...
			Filter(filters.NewAuditingFilter(i.Audit, path.Base(), "GetDefaultIDP")).
	...
	ws.Route(
		ws.GET("/configured/").To(id.findIDPs).
			...
			Filter(filters.NewAuditingFilter(i.Audit, path.Base(), "ListIDPs")).
	...
	ws.Route(
		ws.GET("/configured/{name}").To(id.getIDP).
			...
			Filter(filters.NewAuditingFilter(i.Audit, path.Base(), "GetIDP")).
	...
	ws.Route(
		ws.PUT("/configured/{name}").To(id.putIDP).
			...
			Filter(filters.NewAuditingFilter(i.Audit, path.Base(), "UpdateIDP")).
	...
	ws.Route(
		ws.PUT("/clients/{name}").To(id.putIDPClient).
			...
			Filter(filters.NewAuditingFilter(i.Audit, path.Base(), "UpdateIDPClient")).

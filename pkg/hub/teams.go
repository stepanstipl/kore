...
// Teams is the hub api teams interface
type Teams interface {
	// Delete removes the team from the hub
...
func (t *teamsImpl) List(ctx context.Context) (*orgv1.TeamList, error) {
	...
	if err != nil {
		log.WithError(err).Error("trying to retrieve list of team in hub")
...
// Exists checks if the team exists in the hub
...
func (t *teamsImpl) Update(ctx context.Context, team *orgv1.Team) (*orgv1.Team, error) {
	...
	if !user.IsGlobalAdmin() && !t.IsValidTeamName(team.Name) {
		return nil, ErrNotAllowed{message: "name: " + team.Name + " cannot be used in the hub"}
	}
	...
	logger.Info("attempting to update or create team in hub")
	...
	if err != nil {
		logger.WithError(err).Error("trying to check for team in hub")

		return nil, err
	}

	// @step: update or create in the hub
	...
	if !found {
		...
		if _, err := team, t.Store().Client().Update(ctx,
			...
			log.WithError(err).Error("trying to create / update a team in hub")
	...
	).Event("team has been update or created in the hub")
...
func (t *teamsImpl) IsValidTeamName(name string) bool {
	...
	for _, x := range []string{"kube-", "prometheus", "hub", "istio", "grafana", "olm", "default"} {

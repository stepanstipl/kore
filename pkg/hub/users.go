...
// Users is the hub api users interface
type Users interface {
	// EnableUser is used to create an user in the hub
	EnableUser(context.Context, string, string) error
	// Delete removes the user from the hub
	Delete(context.Context, string) (*orgv1.User, error)
	// Exist checks if the user exists
	Exists(context.Context, string) (bool, error)
	// Get returns the user from the hub
...
func (h *usersImpl) EnableUser(ctx context.Context, username, email string) error {
	...
	logger.Info("enabling the user in the hub")
	...
	if !found {
		logger.Debug("provisioning the user in the hub")

		if err := h.usermgr.Users().Update(ctx, &model.User{
			Username: username,
			Email:    email,
		}); err != nil {
			logger.WithError(err).Error("trying to create the user in the hub")
		...
		if err != nil {
			log.WithError(err).Error("trying to get a count on the hub users")
...
func (h *usersImpl) Delete(ctx context.Context, username string) (*orgv1.User, error) {
	...
	log.WithFields(log.Fields{
		"username": u.Username,
	}).Info("deleting the user from the hub")
	...
	if _, err := h.usermgr.Users().Delete(ctx, u); err != nil {
		log.WithError(err).Error("trying to remove user from hub")
...
func (h *usersImpl) Update(ctx context.Context, user *orgv1.User) (*orgv1.User, error) {
	...
	if err := h.usermgr.Users().Update(ctx, DefaultConvertor.ToUserModel(user)); err != nil {
		log.WithError(err).Error("trying to update the user in the hub")

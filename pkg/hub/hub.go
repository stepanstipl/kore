...
// hubImpl is the implementation for the hub api
type hubImpl struct {
	// auditor is the audit interface
	auditor audit.Interface
	// config is the configuration of the hub
	config *Config
...
func New(sc store.Store, usermgr users.Interface, auditor audit.Interface, config Config) (Interface, error) {
	...
	if config.HasHMAC() {
		log.Warn("no hmac for hub was provided, generating a random one (this has consequences!)")
	...
	// @step: call the setup code for the hub

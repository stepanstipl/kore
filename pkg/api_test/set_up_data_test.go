...
func ensureTeamExists(name string, description string, summary string) error {
	...
	if err != nil {
		...
		case *operations.GetTeamNotFound:
			team := &api_test_models.V1Team{
				Metadata: &api_test_models.V1ObjectMeta{
					Name: name,
				},
				Spec: &api_test_models.V1TeamSpec{
...
func ensureUserExists(username string, email string) error {
	...
	if err != nil {
		...
		case *operations.GetUserNotFound:
			user := &api_test_models.V1User{
				Metadata: &api_test_models.V1ObjectMeta{
					Name: username,
				},
				Spec: &api_test_models.V1UserSpec{
					Disabled: &disabled,
					Username: &username,
					Email:    &email,
				},
...
func ensureUserInTeam(team string, username string) error {
	...
	addTeamMemberParams := operations.NewAddTeamMemberParams().
		WithTeam(team).
		WithUser(username).
		WithBody(&api_test_models.V1TeamMember{
			Spec: &api_test_models.V1TeamMemberSpec{
				Team:     &team,
				Username: &username,
				Roles:    []string{},
			},

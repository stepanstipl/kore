...
func EnsurePrivateSubnet(svc ec2.EC2, vpc VPC, az, cidrBlock string) (_ *ec2.Subnet, _ *ec2.NatGateway, ready bool, _ error) {
	...
	natGateway, ready, err := EnsureNATGateway(svc, vpc, subnet)
	if err != nil {
		return nil, nil, false, err
	}
...
func EnsurePrivateSubnets(svc ec2.EC2, vpc VPC, azs []string, startIP net.IP) (_ []*ec2.Subnet, _ []*ec2.NatGateway, ready bool, _ error) {
	...
	allReady := true
	for _, az := range azs {
		subnet, natGateway, ready, err := EnsurePrivateSubnet(svc, vpc, az, ipNet.String())
		if err != nil {
			return nil, nil, false, err
		}

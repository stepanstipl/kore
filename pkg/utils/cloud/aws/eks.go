...
// TODO: rename to show it's eks specific
...
func (c *Client) Delete() (*eks.DeleteClusterOutput, error) {
	...
	if err != nil {
		if aerr, ok := err.(awserr.Error); ok {
			log.Debugf("aws specific error when trying to delete eks cluster from aws %s", aerr.Error())
		} else {
			log.WithError(err).Debug("error when trying to delete eks cluster from aws")
		}
...
func (c *Client) CreateNodeGroup(nodegroup *eksv1alpha1.EKSNodeGroup) error {

...
// ReconcileDeleteRoles removes the roles
func (a crCtrl) ReconcileDeleteRoles(request reconcile.Request) (reconcile.Result, error) {
...
// ReconcileRoles ensures the clusters roles across all the managed clusters
func (a crCtrl) ReconcileRoles(request reconcile.Request) (reconcile.Result, error) {
	logger := log.WithFields(log.Fields{
		"name":      request.NamespacedName.Name,
		"namespace": request.NamespacedName.Namespace,
	})
	...
	if finalizer.IsDeletionCandidate(role) {
		return a.ReconcileDeleteRoles(request)
	}

...
func (a *awsAutoScaler) Ensure() (reconcile.Result, error) {
	...
	for _, ng := range *a.nodeGroups {
		...
		for _, asgName := range awsNg.Resources.AutoScalingGroups {
			...
			asgValues[*asg.AutoScalingGroupName] = map[string]interface{}{
				"Min": *asg.MinSize,
				"Max": *asg.MaxSize,
			}
	...
	awsAutoScalerService, err := helpers.GetServiceFromAppNameAndValues(
		a.ctx,
		"aws-autoscaler",
		a.kubeCluster,
		"kube-system,",
		map[string]interface{}{
			"AutoScalingImage":   "k8s.gcr.io/cluster-autoscaler:v1.14.7",
			"ASGs":               asgValues,
			"AwsRegion":          *awsClient.Sess.Config.Region,
			"AutoscalingRoleARN": *asgRole.Arn,
		},
	)
	if err != nil {
		return reconcile.Result{}, err

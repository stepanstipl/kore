...
func (a Controller) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if err := a.mgr.GetClient().Get(ctx, request.NamespacedName, cluster); err != nil {
		return controllers.NewReconcileError(err, kerrors.IsNotFound(err)).Result()
	}

	finalizer := kubernetes.NewFinalizer(a.mgr.GetClient(), finalizerName)
	if added, err := finalizer.AddIfNotSet(ctx, cluster); added {
		if err != nil {
	...
	err := func() *controllers.ReconcileError {
		...
		return controllers.ErrNotReady
	}()

	if err != nil && err != controllers.ErrNotReady {
	...
	return err.ResultWithRequeueAfter(5 * time.Second)
}

...
class ClusterBuildForm extends React.Component {
  ...
  componentDidMount() {
    ...
    this.componentDidMountComplete = Promise.resolve().then(async () => {
      ...
      const gcpAccountManagement = (allocations.items || []).filter(a => a.spec.resource.kind === 'AccountManagement')
  ...
  getClusterResource = (values) => {
    ...
    if (values.credential) {
      ...
      selectedCredential = this.state.credentials[this.state.selectedCloud].accountManagement[0]
    }
  ...
  render() {
    ...
    return (
      <div>
        ...
        {selectedCloud ? (
          filteredCredentials.length > 0 || accountManagement.length > 0 ?
